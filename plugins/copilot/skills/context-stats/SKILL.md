...
Add `--since <ISO-8601>` to look at one day and `--by-session` to break the share down per conversation (sessions are a hash of the host conversation id; "-" groups rows the host did not key). Lead with the first line, "kept out of context: N% of requested bytes": requested is what entered context plus what denials kept out. If it says the `postToolUse` hook is not running, report that instead of a saving. Report the table as it is: the numbers are directional (bytes / 4), not a bill, and denied bytes are file sizes at deny time, before any host-side cap. When one tool dominates, say which and suggest the bounded alternative: `head_limit` or `files_with_matches` for searches, `offset`/`limit` for reads, a reducing pipe for shell output, or the `prompt-sift-copilot-worker` for orientation across large files. Set `PROMPT_SIFT_TELEMETRY=0` to stop recording, `PROMPT_SIFT_NUDGE=0` to keep recording without the over-threshold reminder.
